...
class Agent:
    ...
    def save_weights(self, iteration, state_rms,time_name): 
        torch.save({"current_policy_state_dict": self.current_policy.state_dict(),
                    ...
                    "state_rms_mean": state_rms.mean,
                    "state_rms_var": state_rms.var,
                    "state_rms_count": state_rms.count}, "./CheckPoint2/"+self.env_name+"_CP"+time_name+"/"+"finetuning_100_per10000itr_lr6"+ self.env_name  +str(iteration)+ "_weights.pth")

    def load_weights(self):
        	
        if self.env_name == "Ant":
        	checkpoint = torch.load("./"+self.env_name+"200000_weights.pth")
